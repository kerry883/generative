...
export const fetchChats = query({
    ...
    handler:async (ctx ,args)=>{
        ...
        const chats = await ctx.db.query("chats").withIndex("by_user",(q)=>q.eq("userId",user.subject)).order("desc").collect();
        return chats;
    }
})
...
export const generateChatTitle = action({
    ...
    handler: async(ctx ,args)=>{
        ...
        // Get the first few messages to understand context better
        const messages = await ctx.runQuery(api.chat.getChat, {
            chatId: args.chatId
        });
        
        // Build context from first few messages (up to 3)
        let conversationContext = args.userPrompt;
        if (messages && messages.length > 0) {
            const firstMessages = messages.slice(0, 3);
            conversationContext = firstMessages
        ...
        const { text } = await generateText({
            ...
            prompt: `
                Based on this conversation, generate a very short, concise chat title (max 5 words).
